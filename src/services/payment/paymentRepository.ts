...
export class PaymentRepository implements IPaymentRepository {
    ...
    constructor(postgresService: PostgresService, dataSource: DataSource, logger: pino.Logger) {
    ...
    async createTransaction(data: Partial<ITransaction>): Promise<Transaction> {
        try {
            ...
            const savedTransaction = await this.paymentRepository.save(newTransaction);
       ...
       try {
         /*  Start database transaction */
            return await this.dataSource.transaction(async (transactionEntityManager) => {
                ...
                if (!payment) {
                    this.logs.error('Transaction record not found...[CATCH_PAYMENT_REPOSITORY_LEVEL]');
